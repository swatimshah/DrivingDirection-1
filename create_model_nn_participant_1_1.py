...
orig_epochs = loadtxt('final_input_data_with_labels_1_1.csv', delimiter=',', skiprows=1)
print(orig_epochs.shape)

# scale the data with robust scaler
my_epochs = rScaler.fit_transform(orig_epochs[0:92, 0:4096])  # noise
...
# Create the model
model=Sequential()
model.add(Conv1D(filters=20, kernel_size=6, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.25, max_value=0.25), padding='valid', activation='relu', strides=1, input_shape=(64, 64)))
...
# Compile the model   
adam = Adam(learning_rate=0.00018)
